...
resource "aws_iam_role" "stage_nodes_general_iam" {
  # The name of the role
  name = "stage_eks-node-group-general"
...
resource "aws_iam_role_policy_attachment" "amazon_eks_worker_node_policy_general" {
  ...
  role = aws_iam_role.stage_nodes_general_iam.name
}
...
resource "aws_eks_node_group" "stage_nodes_general" {
  # Name of the EKS Cluster.
  cluster_name = aws_eks_cluster.eks.name

  # Name of the EKS Node Group.
  node_group_name = "stage_nodes-general"

  # Amazon Resource Name (ARN) of the IAM Role that provides permissions for the EKS Node Group.
  node_role_arn = aws_iam_role.stage_nodes_general_iam.arn
  ...
  remote_access {
    ec2_ssh_key = "eks-key"
    #source_security_group_ids = [aws_security_group.]
  }
  labels = {
    role = "nodes-general"
  }
